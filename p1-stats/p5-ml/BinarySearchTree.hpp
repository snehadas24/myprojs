...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      ...
      if(node){
          new_node = new Node(node->datum, copy_nodes_impl(node->left), copy_nodes_impl(node->right));
      }
     ...
     // Node *node_ptr = nullptr;
      ...
      if(!node){
          ...
          if(node->left != nullptr && node->right != nullptr){
              Node *node_ptr = new Node(node->datum, node->left, node->right);
              copy_nodes_impl(node->left);
              copy_nodes_impl(node->right);
              return node_ptr;
          }else if(node->left != nullptr){
              Node *node_ptr = new Node(node->datum, node->left, nullptr);
              copy_nodes_impl(node->left);
              copy_nodes_impl(node->right);
              return node_ptr;
          }else{
              Node *node_ptr = new Node(node->datum, nullptr, node->right);
              copy_nodes_impl(node->left);
              copy_nodes_impl(node->right);
              return node_ptr;
          }
     ...
     /* if(node->right){
          node_ptr->right = new Node(node->right->datum, node->right->left, node->right->right);
      }else{
          node_ptr->right = nullptr;
      }
      
      if(node->left){
          node_ptr->left = new Node(node->left->datum, node->left->left, node->left->right);
      }else{
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
      if(!node){
          Node *node_ptr = new Node(item, nullptr, nullptr);
          return node_ptr;
      }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
      ...
      if(less(node->datum, max_element_impl(node->left)->datum)){
          return false;
      }
  ...
  //       about where the element you're looking for could be.
    static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      ...
      if(!node->left && !node->right){
        ...
        if((less(val, node->datum) && less(node->left->datum, val)) || (less(val, node->datum) && node->left == nullptr)){
            return node;
        }
      ...
      if(less(node->datum, val) && node->right){
          return min_greater_than_impl(node->right, val, less);
      }
